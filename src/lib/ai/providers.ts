 ...
 * (server-side) and browser-direct alike.
 *
 * Byte-identical to handai's src/lib/ai/providers.ts (the function that
 * Carmenita's entire LLM story depends on).
 */
  ...
  switch (provider) {
    ...
    case "openrouter": {
      const client = createOpenAI({
        ...
        headers: {
          "HTTP-Referer": "https://carmenita.app",
          "X-Title": "Carmenita Quiz Generator",
        },
      });
      return client(modelId);
    }
