 ...
 * navigation between pages. Reads in useEffect to avoid SSR/hydration
 * mismatches.
 *
 ...
 *
 * Ported byte-identical from handai's src/hooks/useSessionState.ts.
 */
export function useSessionState<T>(
  ...
  // Rehydrate from sessionStorage after mount. We intentionally call
  // setState inside the effect body — this is the standard pattern for
  // reading browser-only APIs without causing SSR hydration mismatches.
  // The React Compiler flags this but it's correct here.
  useEffect(() => {
    try {
